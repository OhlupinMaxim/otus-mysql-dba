...
docker cp ./insert_json.sql otus-mysql-docker-otusdb-1:/insert_json.sql
...
docker exec -it otus-mysql-docker-otusdb-1 chmod 777 /insert_json.sql
...
docker exec -it otus-mysql-docker-otusdb-1 bash -c "mysql -u sb_crm_user -p12345maxim sb_crm < /insert_json.sql"